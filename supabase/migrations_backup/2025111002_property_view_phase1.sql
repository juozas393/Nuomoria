...
DROP POLICY IF EXISTS "Invoice payments manage own" ON public.invoice_payments;
CREATE POLICY "Invoice payments manage own"
    on public.invoice_payments
    for all
    using (auth.uid() = created_by)
    with check (auth.uid() = created_by);

DROP POLICY IF EXISTS "Invoice payments view own" ON public.invoice_payments;
CREATE POLICY "Invoice payments view own"
    on public.invoice_payments
    for select
    using (auth.uid() = created_by);
...
DROP POLICY IF EXISTS "Deposit events manage own" ON public.property_deposit_events;
CREATE POLICY "Deposit events manage own"
    on public.property_deposit_events
    for all
    using (auth.uid() = created_by)
    with check (auth.uid() = created_by);

DROP POLICY IF EXISTS "Deposit events view own" ON public.property_deposit_events;
CREATE POLICY "Deposit events view own"
    on public.property_deposit_events
    for select
    using (auth.uid() = created_by);
