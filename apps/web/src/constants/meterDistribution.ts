...
export function checkPrecondition(
  ...
  if (dist === 'per_person' && ctx.units <= 0) {
    return { allowed: false, reason: 'Nėra butų su gyventojais' };
  }
  if (dist === 'per_consumption' && !ctx.hasMeters) {
...
export const ALLOWED: Record<MeterKind, {
  ...
  water_cold: {
    allowed: ['per_consumption', 'per_area', 'per_apartment', 'per_person', 'fixed_split'],
  ...
  heating: {
    allowed: ['per_area', 'per_consumption', 'per_apartment', 'per_person', 'fixed_split'],
    default: 'per_area',
    hasIndividualMeters: true
  },
  ...
  // Bendri skaitliukai
  electricity_shared: {
    allowed: ['per_apartment', 'per_person', 'per_area', 'fixed_split'],
  ...
  // Fiksuoti mokesčiai
  internet: {
    allowed: ['fixed_split', 'per_apartment', 'per_person'],
    default: 'fixed_split'
  },
  trash: {
    allowed: ['fixed_split', 'per_apartment', 'per_person'],
  ...
  custom: {
    allowed: ['per_apartment', 'per_consumption', 'per_area', 'per_person', 'fixed_split'],
    default: 'per_apartment',
    hasIndividualMeters: true
  },
};
