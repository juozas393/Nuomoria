...
export function MetersSection({ defaultStartDate, meters, onMetersChange }: MetersSectionProps) {
  ...
  const buildMeter = (type: MeterType): MeterForm => {
    ...
    const defaultConfigs: Record<MeterType, { label: string; unit: Unit; price_per_unit: number }> = {
      electricity: { label: 'Elektra', unit: 'kWh', price_per_unit: 0.18 },
      water_cold: { label: 'Vanduo (šaltas)', unit: 'm3', price_per_unit: 2.45 },
      water_hot: { label: 'Vanduo (karštas)', unit: 'm3', price_per_unit: 3.20 },
      electricity_individual: { label: 'Elektra (individuali)', unit: 'kWh', price_per_unit: 0.18 },
      electricity_common: { label: 'Elektra (bendra)', unit: 'kWh', price_per_unit: 0.18 },
      gas: { label: 'Dujos', unit: 'm3', price_per_unit: 0.08 },
      heating: { label: 'Šildymas', unit: 'GJ', price_per_unit: 25.00 },
      internet: { label: 'Internetas', unit: 'Kitas', price_per_unit: 15.0 },
      waste: { label: 'Šiukšlės', unit: 'Kitas', price_per_unit: 8.0 },
      garbage: { label: 'Šiukšlių išvežimas', unit: 'Kitas', price_per_unit: 8.0 },
    ...
    <section className="mt-6">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <QuickChip label="Vanduo (šaltas)" onClick={() => addByType('water_cold')} />
          <QuickChip label="Vanduo (karštas)" onClick={() => addByType('water_hot')} />
          ...
          <div key={m.id} className="rounded-xl border border-neutral-200 bg-white p-3">
            ...
            <div className="mt-2 grid grid-cols-1 sm:grid-cols-6 gap-2">
              <Select 
                ...
                options={[
                  ['water_cold','Vanduo (šaltas)'],
                  ['water_hot','Vanduo (karštas)'],
