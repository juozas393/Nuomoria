...
export const MetersFilters: React.FC<MetersFiltersProps> = React.memo(({
  activeFilter,
  onFilterChange,
  counts
}) => {
  const getCount = (filter: FilterType) => {
    switch (filter) {
      ...
      case 'needs_photo': return counts.needsPhoto;
      case 'needs_reading': return counts.needsReading;
      case 'overdue': return counts.overdue;
      case 'individual': return counts.individual;
      case 'communal': return counts.communal;
      default: return 0;
    }
  };
...
MetersFilters.displayName = 'MetersFilters';
