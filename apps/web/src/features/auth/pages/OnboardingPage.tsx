...
const ALL_STEPS = ['welcome', 'username', 'role', 'stripe', 'complete'] as const;
type Step = 'welcome' | 'username' | 'role' | 'stripe' | 'complete';
...
const OnboardingPage: React.FC<OnboardingPageProps> = ({ userEmail, userId }) => {
  ...
  // ── submit profile (before stripe/complete) ──
  const handleProfileSubmit = async () => {
    ...
    try {
      ...
      if (role === 'landlord') {
        ...
        go('complete');
        setTimeout(() => { window.location.href = '/tenant'; }, 3000);
      }
    } catch {
  ...
  const handleConnectStripe = async () => {
    try {
      ...
      if (!session) { setError('Nėra sesijos'); setConnectingStripe(false); return; }
      ...
      if (!resp.ok) {
        const msg = `Step: ${data.step || '?'} | ${data.detail || data.error || resp.statusText}`;
        alert(msg);
        throw new Error(msg);
      }
      ...
      else throw new Error('Nepavyko gauti Stripe nuorodos');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Stripe klaida');
          ...
          <div className="relative group">
            ...
            >
              ...
              <div className={`transition-all duration-300 ease-out ${transitioning ? 'opacity-0 translate-y-3 scale-[0.98]' : 'opacity-100 translate-y-0 scale-100'}`}>
                ...
                {step === 'role' && (
                  <div className="px-8 pt-9 pb-8 sm:px-10">
                    ...
                    <div className="flex gap-3 mt-7">
                      ...
                      <button onClick={handleProfileSubmit} disabled={!role || loading}
                        className="flex-1 h-[46px] rounded-2xl bg-gradient-to-r from-teal-600 to-teal-500 text-white text-sm font-semibold shadow-[0_4px_20px_rgba(20,184,166,.25)] hover:shadow-[0_8px_28px_rgba(20,184,166,.35)] disabled:opacity-30 disabled:cursor-not-allowed disabled:shadow-none disabled:from-gray-600 disabled:to-gray-500 transition-all duration-200 flex items-center justify-center gap-2">
                        {loading ? (
                ...
                {/* ═══ STRIPE (landlords only) ═══ */}
                {step === 'stripe' && role === 'landlord' && (
                  <div className="px-8 pt-9 pb-8 sm:px-10">
                    <div className="text-center mb-7">
                      <div className="inline-flex items-center justify-center w-[52px] h-[52px] rounded-2xl bg-gradient-to-br from-[#635BFF]/20 to-[#7B73FF]/10 border border-[#635BFF]/15 mb-4">
                        <svg className="w-6 h-6 text-[#A5A0FF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                        </svg>
                      </div>
                      <h2 className="text-[22px] font-bold text-white tracking-tight mb-1">Mokėjimų prijungimas</h2>
                      <p className="text-white/35 text-[13px]">Prijunkite Stripe, kad nuomininkai galėtų mokėti tiesiogiai</p>
                    </div>

                    {/* Stripe Connect info card */}
                    <div
                      className="rounded-2xl border p-5 mb-5"
                      style={{ backgroundColor: 'rgba(99,91,255,0.04)', borderColor: 'rgba(99,91,255,0.12)' }}
                    >
                      <div className="space-y-3">
                        {[
                          { icon: '💳', title: 'Priimkite mokėjimus kortele', desc: 'Visa, Mastercard ir kiti būdai' },
                          { icon: '🏦', title: 'Automatiniai pervedimai', desc: 'Pinigai tiesiai į jūsų banko sąskaitą' },
                          { icon: '📊', title: '3% platformos komisija', desc: 'Skaidrus ir paprastas kainodara' },
                        ].map((item, i) => (
                          <div key={i} className="flex items-center gap-3.5">
                            <div className="w-9 h-9 rounded-xl bg-white/[0.04] flex items-center justify-center flex-shrink-0 text-sm">
                              {item.icon}
                            </div>
                            <div className="min-w-0">
                              <p className="text-white/70 text-[12px] font-medium">{item.title}</p>
                    ...
                    {error && (
                      <div className="mb-4 rounded-2xl border border-red-500/20 bg-red-500/[0.06] px-4 py-3 text-[12px] text-red-400 flex items-center gap-2">
                        <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                        </svg>
                        {error}
                    ...
                    <div className="space-y-3">
                      <button
                        ...
                        className="w-full h-[46px] rounded-2xl bg-gradient-to-r from-[#635BFF] to-[#7B73FF] text-white text-sm font-semibold shadow-[0_4px_20px_rgba(99,91,255,.25)] hover:shadow-[0_8px_28px_rgba(99,91,255,.35)] hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2"
                      >
                        {connectingStripe ? (
                          <><SpinnerIcon className="w-4 h-4" /> Jungiamasi...</>
                        ) : (
                          <>
                      ...
                      <button
                        ...
                        className="w-full h-[44px] rounded-2xl text-white/50 text-[13px] font-medium border border-white/[0.08] hover:text-white/70 hover:bg-white/[0.05] hover:border-white/[0.15] transition-all duration-200"
                      >
                        Praleisti — nustatyti vėliau per Nustatymus
                      </button>
                    </div>
                  </div>
                )}
