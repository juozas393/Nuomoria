...
const TenantSettingsPage: React.FC = () => {
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            ...
            if (updateError) {
                if (import.meta.env.DEV) console.error('Account deletion failed:', updateError.message);
    ...
    const handleRoleChange = async () => {
        ...
        try {
            ...
            if (updateError) {
                if (import.meta.env.DEV) console.error('Role reset failed:', updateError.message);
            ...
            if (import.meta.env.DEV) console.error('Role change error:', e);
                            ...
                            {authProvider === 'email' && (
                                <div className="mt-4 pt-4 border-t border-gray-100">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-gray-900">Susieti su Google</p>
                                    ...
                                    <button
                                        onClick={async () => {
                                            ...
                                            try {
                                                const { error } = await supabase.auth.linkIdentity({ provider: 'google', options: { redirectTo: `${window.location.origin}/tenant/settings` } });
                                                if (error) throw error;
                                            } catch (e: any) {
                                        ...
                                        className="w-full h-9 flex items-center justify-center gap-2 text-sm font-medium text-[#2F8481] bg-[#2F8481]/10 hover:bg-[#2F8481]/20 rounded-lg transition-colors disabled:opacity-50"
                                    >
                                        {linkingGoogle ? (
                                            <><div className="w-4 h-4 border-2 border-[#2F8481] border-t-transparent rounded-full animate-spin" /> Jungiama...</>
                                        ) : (
