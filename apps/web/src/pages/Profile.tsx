...
type AuthProvider = 'google' | 'email' | 'unknown';
...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    if (user && !profileFetched.current) {
      ...
      fetchProfile();
      // Detect auth provider from Supabase session identities
      supabase.auth.getSession().then(({ data }) => {
        const identities = data.session?.user?.identities ?? [];
        const hasGoogle = identities.some(i => i.provider === 'google');
        const hasEmail = identities.some(i => i.provider === 'email');
        setAuthProvider(hasGoogle ? 'google' : hasEmail ? 'email' : 'unknown');
      });
    }
  }, [user]);
  ...
  const fetchProfile = async () => {
    ...
    } catch (e) { if (import.meta.env.DEV) console.error(e); setToast({ message: 'Nepavyko užkrauti', type: 'error' }); }
  ...
  const handleSaveProfile = async () => {
    ...
    } catch (e) { if (import.meta.env.DEV) console.error(e); setToast({ message: 'Nepavyko išsaugoti', type: 'error' }); }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      // Delete profiles row first (separate table, may not be covered by RPC cascade)
      await supabase.from('profiles').delete().eq('id', user.id).then(() => {});

      // Full deletion: public.users + auth.users via SECURITY DEFINER RPC
      const { error: rpcError } = await supabase.rpc('delete_own_account');
      if (rpcError) {
        if (import.meta.env.DEV) console.error('delete_own_account RPC failed:', rpcError.message);
        setToast({ message: 'Klaida trinant paskyrą. Bandykite dar kartą.', type: 'error' });
        setDeletingAccount(false);
        return;
      }

      // Sign out locally (session is already invalid since auth user is gone)
      try { await logout(); } catch { }

    } catch (e: any) {
      if (import.meta.env.DEV) console.error('Delete account error:', e);
    }
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="sticky top-0 bg-gray-950/80 backdrop-blur-xl border-b border-white/[0.06] z-10">
                  ...
                  <div className="flex items-center gap-1.5">
                    {authProvider === 'google' ? (
                      ...
                      <>
                        ...
                        <span className="font-medium text-gray-200">El. paštas</span>
                      </>
                    ) : (
              ...
              {authProvider === 'email' && (
                <div className="mt-4 pt-4 border-t border-white/[0.06]">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-200">Susieti su Google</p>
                      <p className="text-xs text-gray-500">Galėsite prisijungti ir su Google paskyra</p>
                    </div>
                  </div>
                  <button
                    onClick={async () => {
                      ...
                      try {
                        const { error } = await supabase.auth.linkIdentity({ provider: 'google', options: { redirectTo: `${window.location.origin}/profilis` } });
                        if (error) throw error;
                      } catch (e: any) {
                        setToast({ message: 'Klaida: ' + (e.message || 'nepavyko susieti'), type: 'error' });
                        setLinkingGoogle(false);
                      }
                    }}
                    disabled={linkingGoogle}
                    className="w-full h-9 flex items-center justify-center gap-2 text-sm font-medium text-teal-300 bg-teal-500/15 hover:bg-teal-500/25 rounded-lg transition-colors disabled:opacity-50"
                  >
                    {linkingGoogle ? (
                      <><div className="w-4 h-4 border-2 border-teal-300 border-t-transparent rounded-full animate-spin" /> Jungiama...</>
                    ) : (
                      <><svg className="w-4 h-4" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="currentColor" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="currentColor" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="currentColor" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="currentColor" /></svg> Susieti Google paskyrą</>
                    )}
                  </button>
                </div>
              )}
